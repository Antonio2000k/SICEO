<?php
   ...
   // echo '<input type="text" name="esta" value="'.$cambio.'">';
    if($opcion==="cambioModelo"){
        ...
        <?php       
        pg_query("BEGIN");
        $resultado=pg_query($conexion, "SELECT  productos.cnombre, productos.cmodelo,  proveedor.eid_proveedor FROM paproveedor as proveedor INNER JOIN pbproductos as productos ON productos.eid_proveedor = proveedor.eid_proveedor where productos.eid_proveedor='".$cambio."' and productos.bestado='t'");
    ...
    if($opcion==="cambioNombre"){
        pg_query("BEGIN");
        $resultado=pg_query($conexion, "SELECT * from pbproductos where cmodelo='".$cambio."'");
        while ($fila = pg_fetch_array($resultado)) {
